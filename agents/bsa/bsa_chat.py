...
Запуск:
  ...
  python3 bsa_chat.py --mode discuss ...    # ответ на вопрос (от listener)
  python3 bsa_chat.py --tg                  # Telegram-режим (поллинг + ответы)
...
# ── Telegram API (direct send, for --tg mode) ───────────
...
def tg_load_chat_id():
    ...
    cf = BASE / ".tg_chat_id"
    if cf.exists():
        _TG_CHAT_ID = cf.read_text().strip()
        return _TG_CHAT_ID
    return ""


def tg_save_chat_id(cid):
    global _TG_CHAT_ID
    _TG_CHAT_ID = str(cid)
    (BASE / ".tg_chat_id").write_text(str(cid))
...
def tg_send_message(text, parse_mode="HTML"):
    cid = tg_load_chat_id()
    if not cid:
        log("No chat ID configured, using outgoing queue")
        _push_to_telegram(text)
        return False

    safe = _md_to_html(text)
    if len(safe) > 4096:
        _tg_send_long(cid, safe, parse_mode)
        return True

    payload = {"chat_id": int(cid), "text": safe, "disable_notification": False}
    if parse_mode:
...
def _tg_send_long(cid, text, parse_mode):
    chunks = text.split("\n")
...
def discuss_mode(question, direct_send=False):
...
def tg_main():
    """Telegram polling mode — BSA listens and responds directly."""
    if not tg_load_token():
        log("ERROR: No TG_BOT_TOKEN in --tg mode")
        print("ERROR: TG_BOT_TOKEN not found", file=sys.stderr)
        sys.exit(1)
    if not load_key():
        log("ERROR: No DeepSeek API key in --tg mode")
        print("ERROR: DeepSeek API key not found", file=sys.stderr)
        sys.exit(1)

    log("BSA Telegram mode started (polling 5s)")
    offset = 0
        ...
        try:
            ...
            if r and r.get("ok"):
                for upd in r.get("result", []):
                    ...
                    saved = tg_load_chat_id()
                    if not saved:
                        tg_save_chat_id(chat_id)
                        log(f"Chat ID saved: {chat_id}")
                    elif str(chat_id) != saved:
                        log(f"Ignored unknown chat: {chat_id}")
                        continue

                    log(f"TG msg: {text[:100]}")

                    if text == "/start":
                        tg_send_message(
                            "🤖 BSA — Business Strategy Advisor\n\n"
                            "Пиши любые вопросы по стратегии, контенту, конкурентам.\n"
                            "Команды: /new — новый разговор, /help — справка"
                        )
                        continue

                    if text in ("/help", "/new"):
                        if text == "/new":
                            log("TG: /new — new session started on next message")
                        tg_send_message(
                            "🤖 *BSA Telegram*\n\n"
                            "— Любой текст → BSA анализирует\n"
                            "— /new — сбросить контекст\n"
                            "— Работает с Obsidian, конкурентами, стратегией"
                        )
                        continue
                    ...
                    # Add ээ prefix if not already, for compatibility with discuss_mode
                    if not text.startswith("ээ") and not text.startswith("\u044d\u044d"):
                        question = f"ээ {text}!"
                    else:
                        question = text

                    discuss_mode(question, direct_send=True)
